...
from applicant.core.ids import ApplicationId, CampaignId, FollowUpId, OutcomeEventId, RejectionSignalId, SubmissionSnapshotId, new_id
...
class PostSubmissionService:
    ...
    def process_rejection_signal(self, application_id, *, source, signal_text="", confidence=1.0, detail=None):
        signal = RejectionSignal(id=RejectionSignalId(new_id()), application_id=application_id, source=source, signal_text=signal_text, confidence=confidence, detail=detail or {})
    ...
    def scan_email_for_rejection(self, email_subject, email_body, application_id):
        keywords = ["unfortunately", "regret to inform", "not moving forward", "other candidates", "not selected", "position has been filled", "will not be proceeding"]
        combined = (email_subject + " " + email_body).lower()
        matched = [kw for kw in keywords if kw in combined]
        if not matched: return None
        confidence = min(0.5 + 0.1 * len(matched), 0.95)
        return self.process_rejection_signal(application_id, source=RejectionSource.EMAIL, signal_text=f"Email matched: {\", \".join(matched)}", confidence=confidence, detail={"subject": email_subject, "matched_keywords": matched})

    def check_ghosting(self, campaign_id, *, sla_days=14, now=None):
        now = now or datetime.now(UTC)
        signals = []
        for app in self._storage.applications.list_by_status(campaign_id, (ApplicationState.AWAITING_RESPONSE, ApplicationState.POST_SUBMISSION)):
            age = self._submission_age(app, now)
            if age is None or age.days < sla_days: continue
            ghost = GhostingSignal(campaign_id=campaign_id, application_id=app.id, sla_days=sla_days, submission_age_days=age.days, detail={"status": app.status.value, "sla_days": sla_days, "age_days": age.days})
        ...
        if signals: self._storage.commit()
        return signals

    def _submission_age(self, application, now):
        for ev in self._storage.outcomes.list_for_application(application.id):
            if ev.type == "submitted": return timedelta(0)
        return None

    def schedule_follow_up(self, application_id, *, template, delay_hours=None, subject="", body=""):
        app = self._storage.applications.get(application_id)
        if app is None: raise ValueError(f"Application {application_id} not found")
        now = datetime.now(UTC)
        if delay_hours is None:
            delay_hours = THANK_YOU_DELAY_HOURS if template == FollowUpTemplate.THANK_YOU else CHECK_IN_DELAY_DAYS * 24
        fup = FollowUp(id=FollowUpId(new_id()), campaign_id=app.campaign_id, application_id=application_id, template=template, subject=subject or self._default_subject(template), body=body or self._default_body(template), scheduled_at=now + timedelta(hours=delay_hours))
        self._storage.follow_ups.add(fup); self._storage.commit()
        return fup
    ...
    def send_scheduled_follow_ups(self, now=None):
        ...
        for fup in self._storage.follow_ups.list_due(now):
            if self._notification:
                try: self._notification.notify(title=fup.subject, body=fup.body, deep_link=f"/applications/{fup.application_id}")
        ...
        if sent: self._storage.commit()
        return sent

    @staticmethod
    def _default_subject(template):
        if template == FollowUpTemplate.THANK_YOU: return "Thank you for your time"
        if template == FollowUpTemplate.CHECK_IN: return "Checking in on my application"
        if template == FollowUpTemplate.REJECTION_FOLLOW_UP: return "Following up on your decision"
        return "Application follow-up"

    @staticmethod
    def _default_body(template):
        if template == FollowUpTemplate.THANK_YOU: return "Thank you for the opportunity to apply. I look forward to hearing about next steps."
        if template == FollowUpTemplate.CHECK_IN: return "I wanted to check in on the status of my application. Please let me know if you need any additional information."
        if template == FollowUpTemplate.REJECTION_FOLLOW_UP: return "I understand the position may not be a fit, but I would appreciate any feedback you could share."
        return ""
    ...
    def archive(self, application_id):
        ...
        try:
            ...
            self._storage.applications.update(app); self._storage.commit()
            return app
        except Exception: return None

    def _record_outcome_event(self, application, outcome_type):
        event = OutcomeEvent(id=OutcomeEventId(new_id()), application_id=application.id, type=outcome_type, source=OutcomeSource.AUTO)
