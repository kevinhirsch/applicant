...
* :class:`FakePageSource` â€” an in-memory model walked from an
  ...
  nothing installed.
* :class:`PlaywrightPageSource` â€” the REAL driver. It imports patchright/Playwright
...
from __future__ import annotationsimport reimport socketimport timefrom typing import Protocol, runtime_checkablefrom urllib.parse import urlsplitfrom applicant.adapters.browser.ats import AtsAdapter, FakePage, resolve_atsfrom applicant.core.errors import InvalidInputfrom applicant.core.rules.url_safety import ip_chain_is_blocked, scheme_is_allowedfrom applicant.ports.driven.browser_automation import DetectedField, PageStatedef url_safety_violation(url: str) -> str | None:
    ...
    Shared core of the SSRF guard: require an http(s) scheme and resolve the host â€”
...
def assert_navigable_url(url: str) -> None:
    ...
    require an http(s) scheme and resolve the host â€” if it (or ANY address it
...
class PageSource(Protocol):
    ...
    def set_input_files(self, selector: str, file_path: str) -> None:
        ...
        A deterministic, idempotent pre-fill step â€” uploading the rendered base
        rÃ©sumÃ©, not a submit â€” so it never crosses the pre-fill-stop boundary.
        """
        ...
    ...
    def is_account_gate(self) -> bool:
        """True at the account step (sign-in OR create-account) â€” hand off / log in."""
        ...
...
class PlaywrightPageSource:
    ...
    * ``camoufox`` (the default) â€” a Firefox-based anti-detect browser that injects
    ...
    * ``chromium`` â€” patchright (a Playwright fork that strips automation tells) +
    ...
    NOTE: clicks/submits are NOT performed here â€” the adapter routes the boundary
    ...
    def __init__(
        ...
        *,
        ...
        #: launching a local browser. Persona is then ``native`` â€” NO fingerprint
        ...
        # (camoufox engine) â€” exactly one is set; teardown closes whichever launched.
        self._pw = None
        self._cam = None
        self._context = None
        self._page = None
    ...
    def _launch_chromium(self, *, headless: bool | str, user_data_dir: str) -> None:
        ...
        try:  # pragma: no cover - integration-gated
            if self._cdp_endpoint:
                ...
                # persona) â€” that would only RISK incoherence with a real OS.
    ...
    def _safe_teardown(self) -> None:  # pragma: no cover - integration-gated
        ...
        the remote browser is the Windows VM's Chrome â€” we only DISCONNECT (close our
    ...
    def fingerprint_init_script(fingerprint: dict[str, str]) -> str:
        ...
        (FR-STEALTH-1). It does NOT touch ``Sec-CH-UA`` â€” real Google Chrome emits
    ...
    #: without this flag ``canvas.getContext('webgl')`` returns ``null`` â€” and a real
    ...
    def camoufox_options(
        ...
        *,
        ...
        coherent fingerprint (BrowserForge), so â€” unlike the chromium path â€” no
        ...
        # inside the display-less container â€” that is headful (no headless detection
    ...
    #: After the first Apply click, prefer a MANUAL application over rÃ©sumÃ©-autofill or
    ...
    def _settle(self, timeout_ms: int = 12_000) -> None:  # pragma: no cover - integration-gated
        ...
        ``load`` is blank. Wait for network to go idle â€” bounded + best-effort
    ...
    def enter_application(self) -> PageState | None:  # pragma: no cover - integration-gated
        ...
        no entry control). A benign navigation â€” never an irreducible human step."""
        if not self._click_first(self._APPLY_SELECTORS):
            return None
        self._settle()
        # Workday usually offers "Apply Manually" alongside rÃ©sumÃ©-autofill / 3rd-party
    ...
    def log_in(self, username: str, password: str) -> bool:  # pragma: no cover - integration-gated
        ...
        False â€” the caller then holds the sandbox + hands off. Never raises.

        Does NOT drive OAuth ("Sign in with Google") â€” that path is handled separately
    ...
    #: produce it â†’ 2FA hand-off). Broad on purpose: Google phrases this many ways.
    _TWO_FACTOR_MARKERS = (
        "2-step verification",
        "2-step",
        "two-factor",
    ...
    def log_in_with_google(self, username: str, password: str) -> str:  # pragma: no cover
        ...
        * ``"ok"``       â€” authenticated (a live persistent Google session carried us
                           through, or the email/password were accepted) and we are past
                           the gate;
        * ``"two_factor"`` â€” Google is demanding a second factor the engine cannot
                           produce â†’ the caller runs the 2FA notify/continue/retry flow;
        * ``"failed"``   â€” could not complete (caller hands off).
    ...
    #: reCAPTCHA script is NOT one of these â€” only a visible challenge counts, so the
    ...
    def detect_fields(self) -> list[DetectedField]:  # pragma: no cover
        ...
        for handle in self._page.query_selector_all("input, select, textarea"):
            # A typeable ARIA combobox (react-select) is an <input> too â€” skip it here
            ...
            # selector â€” fields never resolved. Build ``[name="..."]`` / ``#id``.
        ...
        # ARIA combobox) â€” NOT <select>, so the query above misses them entirely.
    ...
    def _best_label(handle) -> str:  # pragma: no cover - integration-gated
        ...
        Order: ``aria-label`` â†’ ``aria-labelledby`` target â†’ the associated
        ``<label>`` (``el.labels`` / wrapping label) â†’ ``placeholder`` â†’ ``title``.
        Most real ATS forms (Greenhouse, Lever, iCIMS) label fields with a separate
        ``<label for=â€¦>`` element, NOT ``aria-label`` â€” without this the engine only
    ...
    def type_value(
        ...
        # A <select> dropdown cannot be typed into â€” it must be chosen via
        ...
        # operated by clicking it open and choosing the matching option â€” typing into
        # it does nothing.
    ...
    def set_input_files(self, selector: str, file_path: str) -> None:  # pragma: no cover
        # Attach the rendered base rÃ©sumÃ© to the file input (FR-RESUME-4). Playwright's
        # set_input_files drives the native file chooser without opening an OS dialog.
        self._page.set_input_files(selector, file_path)

    #: Phrasings that all mean "decline / prefer not to answer" â€” so the user's stored
    ...
    def _select_option(self, selector: str, value: str) -> None:  # pragma: no cover
        ...
        tolerant text match â€” substring overlap or same decline-intent). Raises if
    ...
    def _is_combobox(handle) -> bool:  # pragma: no cover
        """True if a handle is an ARIA dropdown â€” a custom <button> listbox OR a
    ...
    def _is_listbox_button(self, selector: str) -> bool:  # pragma: no cover
        """True if ``selector`` is a custom dropdown â€” a button that opens a listbox,
        or an ARIA combobox (incl. a typeable react-select input) â€” vs a plain input."""
        el = self._page.query_selector(selector)
    ...
    def _choose_listbox_option(self, selector: str, value: str) -> None:  # pragma: no cover
        ...
        Handles BOTH kinds of ARIA dropdown: a <button> listbox (Workday â€” click opens,
        ...
        trigger.click()
        # 1. Match the options shown on open (no typing) â€” catches short lists + the
        ...
        #    filter query â€” the first couple of meaningful words â€” to narrow it down
        #    (typing the full value, e.g. "United States of America", often matches an
        #    option labelled "United States +1" literally â†’ nothing), then match.
    ...
    #: page. "Sign in with Google/LinkedIn" are OAuth flows the engine cannot drive â€”
    ...
    def is_account_gate(self) -> bool:  # pragma: no cover
        """True when the page is the account step â€” the user (or a stored credential)
