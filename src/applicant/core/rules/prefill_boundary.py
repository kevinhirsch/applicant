...
#: Steps that are always irreducible human steps — the engine must hand off.
...
def is_irreducible_human_step(step: StepKind) -> bool:
    """True if ``step`` is an irreducible human step the engine must never perform.

    Note: ``FINAL_SUBMIT`` is *conditionally* irreducible — see
    ``ensure_action_allowed`` — so it is not in the unconditional set.
    """
    return step in _IRREDUCIBLE


def ensure_action_allowed(step: StepKind, *, engine_submit_authorized: bool = False) -> None:
    """Raise ``PrefillBoundaryViolation`` if the engine may not perform ``step``.

    Args:
        step: the action the engine is about to take.
        engine_submit_authorized: True only when the user has explicitly
            authorized the engine to click the *final* submit (FR-PREFILL-5).
    """
