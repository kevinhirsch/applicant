...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("APPLICANT_SCRIPT_HOST", "localhost")).strip()
...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip Applicant-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get("X-Applicant-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-Applicant-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "applicant-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (applicant):")
        ...
        if new_urgent:
            ...
            # straight into the Applicant Email tab).
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on APPLICANT_SCRIPT_HOST",
