...
def setup_applicant_routes() -> APIRouter:
    ...
    @router.get("/features")
    async def applicant_features() -> dict:
        """Derived Applicant section state (engine setup status + dormant registry).

        Public, like ``/api/auth/features``: it leaks no secrets — only which
        sections are active/configured/locked/disabled. ``compute_features`` is
