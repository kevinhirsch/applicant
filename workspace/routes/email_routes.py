...
def _apply_applicant_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Applicant-Origin"] = APPLICANT_MAIL_ORIGIN
    if kind:
        msg["X-Applicant-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Applicant-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
            ...
            if entry:
                ...
                if (now - last_used) < _IMAP_IDLE_MAX:
                    try:
                        ...
                        logger.warning("Bare exception in email_routes.py")
                        try: conn.logout()
                        except Exception as _e: logger.warning("email cleanup %s.%s() failed: %s", "conn", "logout", _e)
                        del _IMAP_POOL[pool_key]
                else:
                    try: conn.logout()
                    except Exception as _e: logger.warning("email cleanup %s.%s() failed: %s", "conn", "logout", _e)
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Applicant marker header, but include the subject
                # fallback too. The fallback uses a distinct Applicant prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Applicant-Kind "reminder" SUBJECT "Reminder (Applicant):"{from_clause})',
                )
    ...
    @router.delete("/applicant/reminders")
    async def delete_applicant_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Applicant reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Applicant-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Applicant):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Applicant):")})'))
                            # Legacy reminders created before the Applicant
            ...
            logger.error(f"delete_applicant_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", applicant_kind=None, applicant_ref=None,
    ):
        ...
        _apply_applicant_headers(outer, applicant_kind or "scheduled", applicant_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, applicant_kind)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("applicant_kind") or "scheduled",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="applicant.local")

        if req.in_reply_to:
            outer["In-Reply-To"] = req.in_reply_to
        if req.references:
            outer["References"] = req.references
        if req.applicant_kind:
            _apply_applicant_headers(outer, req.applicant_kind)
        ...
        if smtp_host:
            ...
            try:
                ...
                try:
                    ...
                    try: smtp.quit()
                    except Exception as _e: logger.warning("email cleanup %s.%s() failed: %s", "smtp", "quit", _e)
            except Exception as e:
