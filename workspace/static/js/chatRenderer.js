...
const _ACTION_RECENTS_KEY = 'applicant-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'applicant-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Applicant';
          ...
          try {
            ...
            compactBody.innerHTML = '<span style="color:var(--red);">Compaction failed: ' + uiModule.esc(String(err.message)) + '</span>';
          }
        });
      }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Applicant' : shortModel(resolvedModel);
