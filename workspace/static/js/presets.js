...
export const PROMPT_TEMPLATES = [
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are a supportive, practical job-search coach. Help the user find roles, tailor their resume and cover letters, prepare for interviews, and stay organized through their search. Give clear, specific, actionable advice grounded in what the user is actually applying for. Be encouraging but honest about tradeoffs, and always keep the user's stated goal at the center."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = JSON.parse(localStorage.getItem('applicant-hidden-presets') || '[]');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('applicant-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = JSON.parse(localStorage.getItem('applicant-hidden-presets') || '[]');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = JSON.parse(localStorage.getItem('applicant-char-sessions') || '{}');
      charSessions[sessionId] = charName;
      localStorage.setItem('applicant-char-sessions', JSON.stringify(charSessions));
