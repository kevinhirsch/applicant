...
const BADGE_SEEN_KEY = 'applicant-calendar-badge-seen';
...
function _headerHTML() {
  ...
  <div class="cal-quickadd-row" id="cal-quickadd-row">
    <input
      ...
      autocomplete="off"
    />
    <span class="cal-quickadd-hint" id="cal-quickadd-hint" aria-hidden="true"><span class="qa-hint-accent">Quick add</span> — interview with Acme 1pm tmrw <svg class="qa-hint-enter" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('applicant.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('applicant.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('applicant.cal.detailH'); } catch {}
      };
...
const LS_KEY = 'applicant-calendar-cache';
