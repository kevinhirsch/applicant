...
function _publishActiveAccount() {
  try { window.__applicantActiveEmailAccount = state._libAccountId || null; } catch (_) {}
...
export function openEmailLibrary(opts = {}) {
  ...
  modal.innerHTML = `
    <div class="modal-content doclib-modal-content" style="--window-w:720px;max-height:85vh;background:var(--bg);">
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete Applicant reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show Applicant reminder emails">
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all Applicant reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/applicant/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
const _BUBBLES_DISABLED_KEY = 'applicant.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'applicant.email.summaryCollapsed';
...
async function _openEmailAsTab(em, folder) {
  ...
  modal.id = modalId;
  modal.innerHTML = `
    <div class="modal-content doclib-modal-content email-reader-tab-content" style="background:var(--bg);--window-w:720px;max-height:85vh;display:flex;flex-direction:column;">
      <div class="modal-header">
...
async function _openEmailWindow(em, folder) {
  ...
  modal.style.cssText = 'pointer-events:none;background:transparent;';
  modal.innerHTML = `
    <div class="modal-content email-window-content" style="--window-w:640px;max-height:80vh;display:flex;flex-direction:column;background:var(--bg);">
      <div class="modal-header">
        <h4 style="display:flex;align-items:center;gap:6px;min-width:0;flex:1;">
