// ============================================
// Applicant UI — Main Application Orchestrator
...
const notesModule = null; // notes integration removed (#303); guards below no-op safely
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__applicantDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (_opener) window._applicantRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('applicant-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'applicant-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Applicant...');
      }
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'applicant-toolbar-visibility';
    try {
...
// ============================================
function startApplicantApp() {
  if (window.__applicantAppStarted) return;
  window.__applicantAppStarted = true;
  // Set CSS variables
  ...
  window._applicantBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._applicantRouteOpener) {
          try { window._applicantRouteOpener(); } catch (_) {}
          window._applicantRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startApplicantApp, { once: true });
} else {
  startApplicantApp();
}
