Feature: SSRF guard covers the entry URL and every redirect / subresource hop
  ...
  # residual gap (#310): a public host that 3xx-redirects to an internal/metadata
  # host, and subresource requests, were not re-validated. The browser adapter now
  # installs a per-request route guard (PlaywrightPageSource._guard_route) reusing
  # the same core rule (ip_chain_is_blocked) on every hop — the redirect/subresource
  # scenarios below now assert that guard aborts a blocked hop.
