# Issue #337 — FakePageSource ↔ PlaywrightPageSource parity / adapters/browser/page_source.py
#   All @pending scenarios below are now active. FakePageSource.enter_application
#   advances past posting pages; log_in respects the login_fails flag; offers_google_signin
#   delegates to the page model; is_account_gate already detects sign-in-only pages.
...
# GREEN: the fake now mirrors the real driver's decision branches for these paths.

Feature: The fake page source approximates the real driver's decision branches

  Scenario: The fake now clicks Apply on a posting page (mirrors the real driver)
