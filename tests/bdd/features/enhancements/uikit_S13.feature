# Issue #482 — FR-UIKIT-2 — workspace/static/js/applicantVault.js
# Masked Elements fields + a Window-kit add/edit modal.
# Part of the FR-UIKIT component-kit migration (epic #458). See docs/spec/ui-kit-migration.md.

Feature: Map the Credential vault onto Elements + Window

  Scenario: S13 baseline — the pre-migration anchor holds today
    Given the UI-kit migration item "S13"
    Then its pre-migration baseline anchor is satisfied today

  Scenario: S13 target — the surface is migrated onto the vendored kit
    Given the UI-kit migration item "S13"
    Then its post-migration kit target is satisfied
