...
# production-mode preset (e.g. APPLICANT_MODE=production) that flips them together does
# not exist yet.
  ...
  Scenario: A fresh install does no real outbound work until each integration is opted in
    ...
    And the orchestrator runs the in-process shim and the scheduler is idle
