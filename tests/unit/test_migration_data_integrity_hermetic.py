"""Hermetic forward-migration data-integrity tests (#365).

These tests run in the default CI suite (no ``@pytest.mark.integration``).
They use a temp-file SQLite database so no Postgres is required.

Strategy to prevent alembic/env.py from routing to Postgres:
  alembic/env.py URL precedence:
    1. ``-x db_url=...`` (from cmd_opts.x)  ← we use this
    2. ``DATABASE_URL`` env var
    3. ``get_settings().database_url``
    4. alembic.ini placeholder

  Passing ``cmd_opts`` with ``x=["db_url=<sqlite_url>"]`` makes option (1)
  win unconditionally, so neither the environment variable nor the settings
  object can redirect the connection to a real Postgres server.

Two variants mirror the integration tests in test_migration_data_integrity.py
but run without Postgres:
  1. From revision 0001_initial → head
  2. From revision 0007_pii_retention_timestamps → head

The ``_assert_schema_matches_models`` helper is identical to the one in the
integration file — it checks that every ORM-declared table/column is present in
the migrated DB (and vice-versa), ensuring no drift between models and alembic.
"""
...
# ---------------------------------------------------------------------------
# No pytestmark = pytest.mark.integration here — these tests MUST run in CI.
# ---------------------------------------------------------------------------


def _make_alembic_config(db_url: str) -> Config:
    """Return an alembic Config pinned to *db_url* on SQLite.

    The ``-x db_url=...`` trick (passed via ``cmd_opts``) takes top priority
    in alembic/env.py — it wins over both ``DATABASE_URL`` and
    ``get_settings().database_url``, so no Postgres connection is ever
    attempted regardless of the environment.
    """
    # Build a minimal Namespace that mimics what argparse produces when
    # ``alembic -x db_url=<url> upgrade head`` is invoked on the CLI.
    cmd_opts = argparse.Namespace(x=[f"db_url={db_url}"])

    cfg = Config(str(_PROJECT / "alembic.ini"), cmd_opts=cmd_opts)
    # set_main_option is a belt-and-suspenders backup; the x-arg is the real guard.
...
def test_hermetic_migrate_from_0001_to_head_schema_matches():
    """Hermetic: run the full 0001 → head chain on empty SQLite, assert schema matches models.

    SQLite does not allow ``ALTER TABLE ADD COLUMN DEFAULT CURRENT_TIMESTAMP NOT NULL``
    on a table that already has rows (non-constant default; used in 0007).  To avoid
    this limitation the full chain is run on an empty database (no rows between
    migration steps), and data is inserted AFTER ``upgrade head``.  This verifies:

    (a) every migration in the 0001 → head chain applies without error on SQLite
    (b) the final schema matches every ORM-declared table and column
    (c) all seeded rows survive a round-trip read with correct values

    The -x db_url trick in _make_alembic_config ensures alembic/env.py never
    routes to Postgres regardless of the DATABASE_URL environment variable.
    """
    ...
    try:
        ...
        # Run the full chain on an empty DB — 0007's ADD COLUMN succeeds because
        # there are no rows yet (non-constant default only fails with existing rows).
        command.upgrade(cfg, "head")

        # Seed data into the fully-migrated schema and verify round-trip.
...
def test_hermetic_migrate_from_0007_to_head_data_survives():
    """Hermetic: run up to 0007 on empty SQLite, seed data, upgrade to head.

    Starts from a completely empty database.  ``command.upgrade`` to
    ``0007_pii_retention_timestamps`` runs the full chain 0001→0007 from
    scratch.  Data is seeded into the 0007 schema (no ``seq`` column yet,
    normalized/redline_state columns may be present), then 0008 is applied.
    The -x db_url trick in _make_alembic_config ensures alembic/env.py never
    routes to Postgres.
    """
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as tf:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            # 0008_schema_hardening adds seq to agent_runs and backfills to 0
            row = session.execute(
            ...
            # 0008 adds uq_onboarding_profiles_campaign
            ucs = {uc["name"] for uc in
