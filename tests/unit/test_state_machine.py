...
LEGAL = [
    ...
    # FR-PREFILL-4: a mid-flow account-creation page hands off from PREFILLING.
    (S.PREFILLING, S.AWAITING_ACCOUNT_HUMAN_STEP),
    ...
    (S.SUBMITTED_BY_USER, S.POST_SUBMISSION),
    (S.FINISHED_BY_ENGINE, S.POST_SUBMISSION),
    (S.POST_SUBMISSION, S.AWAITING_RESPONSE),
    (S.POST_SUBMISSION, S.REJECTED),
    (S.POST_SUBMISSION, S.GHOSTED),
    (S.AWAITING_RESPONSE, S.REJECTED),
    (S.AWAITING_RESPONSE, S.GHOSTED),
    (S.AWAITING_RESPONSE, S.FOLLOWING_UP),
    (S.AWAITING_RESPONSE, S.ARCHIVED),
    (S.REJECTED, S.ARCHIVED),
    (S.GHOSTED, S.FOLLOWING_UP),
    (S.GHOSTED, S.ARCHIVED),
    (S.FOLLOWING_UP, S.REJECTED),
    (S.FOLLOWING_UP, S.GHOSTED),
    (S.FOLLOWING_UP, S.AWAITING_RESPONSE),
    (S.FOLLOWING_UP, S.ARCHIVED),
]
...
ILLEGAL = [
    ...
    (S.SUBMITTED_BY_USER, S.PREFILLING),  # illegal: only POST_SUBMISSION is allowed next
]
