...
def test_scoring_biases_toward_converting_signature(storage, embedding, campaign):
    ...
    # signature-alignment lift, unrelated to role-domain-fit. Under
    # role_domain_fit's ALLOWLIST posture (round 2) the title must plainly
    # match an in-domain role family or the gate short-circuits before
    # signature alignment ever runs.
    posting = JobPosting(
        id=JobPostingId(new_id()), campaign_id=campaign.id,
