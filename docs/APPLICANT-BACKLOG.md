...
- **STATUS (2026-08-11):** ✅ **Backend + first UI surface shipped** — `ScoreBreakdown` entity + `ExplainService` (deterministic weighted factors reconciling with the persisted score, ZERO-weight optional LLM nuance, lazy-compute+cache on `rationale['breakdown']`) + `GET /api/explain/{id}` + explain proxy; **"Why this score?" greens/reds panel now live in the digest review queue** (`digest.html`: coloured dot + signed contribution + detail + reconciling summary), browser-verified on 10.0.1.11 (`f49ff9c32`). Reliability fix: endpoint path is deterministic-only so a wedged LLM can't hang the GET (was timing out at 10s). **Remaining:** compact top-plus/top-minus summary on the Today/Pending-Reviews command-center cards; the same panel inside the Review modal (`documents.html`); actionable factor-thumbing → FeedbackService (ties FS-5, RUX-5).
