...
| **Campaign settings / switcher** | Settings → "Campaign" (`mountApplicantCampaignSettings`); shared switcher embedded in the Today/Tracker headers + the daily-updates picker (`applicantCampaignSwitcher.js`, P1-10 — no standalone nav door) | Create ("Start a search")/rename/archive/duplicate/delete campaigns; upload each campaign's **own base résumé** (per-campaign onboarding intake); set run mode (continuous / fixed duration / until N viable), daily throughput (1–30, capped), exploration budget; toggle per-campaign discovery sources with learned yield stats; switch/filter Today, Tracker, and daily updates by search. | `applicant_campaigns_routes.py` (incl. `POST` create) + `applicant_setup_routes.py` (per-campaign base résumé) → engine campaign config + discovery sources + onboarding intake | `FR-AGENT-1/2`, `FR-CRIT-4`, `FR-DISC-2/5`, `NFR-EXT-1` | `.admin-card`, `.memory-badge`, `.memory-sort-select`, `.settings-row`/`.settings-label`, `.cs-*` |
