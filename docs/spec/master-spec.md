...
- **FR-PREFILL-2a (MUST — universal ATS support, provider-agnostic):** Pre-fill MUST work on **any** ATS/job-application form — Workday, Greenhouse, Lever, Ashby, iCIMS, SmartRecruiters, bespoke forms — driven **generically by live-DOM inspection + intelligence (the LLM)**, NOT by per-vendor hardcoded selectors. The generic driver MUST: (a) detect every fillable control regardless of widget kind — text/email/tel inputs, `<select>`, **custom `<button>`/listbox dropdowns** (e.g. Workday), and **typeable `role=combobox` / react-select** widgets (e.g. Greenhouse/Lever); (b) **resolve each field's human label** the way a person reads it — `aria-label` → `aria-labelledby` → the associated `<label>` → placeholder/title — then map via attributes + LLM escalation (`FR-PREFILL-3`); (c) walk **single-page** applications (form + Submit on one page) as well as **multi-step** ones, filling before deciding a page is the final-submit page; (d) treat the DOM's **required** marking as authoritative — skip optional unmapped fields, soft-error/ask only on genuinely required ones (`FR-ATTR-5`); (e) skip file-upload controls (résumé/cover-letter upload is `FR-RESUME`, a separate step). Vendor-specific adapters MAY add precision on top, but the engine MUST never *depend* on a vendor adapter to fill a form. Validated live against Greenhouse + Lever; Workday via a faithful real-DOM fixture.
