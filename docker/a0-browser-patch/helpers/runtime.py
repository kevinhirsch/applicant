...
class _BrowserRuntimeCore:
    ...
    def __init__(self, context_id: str):
        ...
        # Residential-proxy escalation state (sticky for this runtime's lifetime).
        self._use_residential_proxy = False
        self._residential_escalation_attempted = False
    ...
    async def _start(self) -> None:
        ...
        if self._use_residential_proxy:
            ...
            if proxy_server:
                ...
                # Stop WebRTC from leaking the real IP around the proxy.
                launch_kwargs["args"] = [
                    *launch_kwargs.get("args", []),
                    "--force-webrtc-ip-handling-policy=disable_non_proxied_udp",
                ]
                PrintStyle.warning(
                    f"Browser using residential proxy {proxy_server} "
                    f"for context {self.safe_context_id}."
                )
        try:
    ...
    async def open(self, url: str = "") -> dict[str, Any]:
        ...
        if target_url and target_url != "about:blank":
            ...
            response = await self._goto(page, normalized)
            if await self._should_escalate_to_residential(response, page):
                await self._activate_residential_proxy()
                await self.ensure_started()
                self._ensure_can_open_page()
                page = await self.context.new_page()
                browser_page = await self._register_page(page)
                self.last_interacted_browser_id = browser_page.id
                await self._goto(page, normalized)
        else:
    ...
    async def _page_block_signature(self, page: Any) -> bool:
        try:
        ...
        try:
            ...
            body = ""
        haystack = f"{title}\n{body}".lower()
        return any(sig in haystack for sig in self._RESIDENTIAL_BLOCK_SIGNATURES)

    async def _is_hard_block(self, response: Any, page: Any) -> bool:
        status = None
        try:
        ...
        if status in (403, 429):
            return True
        if await self._page_block_signature(page):
            return True
        if status == 503:
            return True
        return False

    async def _should_escalate_to_residential(self, response: Any, page: Any) -> bool:
        if self._use_residential_proxy or self._residential_escalation_attempted:
            return False
        proxy_cfg = get_residential_proxy_config()
        if not proxy_cfg.get("enabled") or not proxy_cfg.get("server"):
            return False
        return await self._is_hard_block(response, page)

    async def _activate_residential_proxy(self) -> bool:
        """Relaunch the browser context through the residential proxy.

        Sticky for the rest of this runtime's life so a browse -> apply flow
        shares one residential IP. Leaves a live context on both paths.
        Returns True if the proxied context is live, False if it fell back.
        """
        self._residential_escalation_attempted = True
        self._use_residential_proxy = True
        try:
            await self._discard_stale_context(
                "Escalating Browser session to residential proxy after a hard block."
            )
            await self.ensure_started()
            return True
        except Exception as exc:
            PrintStyle.warning(f"Residential proxy escalation failed: {exc}")
            self._use_residential_proxy = False
            try:
                await self.ensure_started()
            except Exception:
