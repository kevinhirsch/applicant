...
RESIDENTIAL_PROXY_ENABLED_KEY = "residential_proxy_enabled"
RESIDENTIAL_PROXY_SERVER_KEY = "residential_proxy_server"
DEFAULT_RESIDENTIAL_PROXY_SERVER = "http://10.8.0.1:8880"
DEFAULT_RESIDENTIAL_PROXY_ENABLED = True
# Environment overrides take precedence over plugin settings when set:
RESIDENTIAL_PROXY_ENV_SERVER = "A0_BROWSER_RESIDENTIAL_PROXY"
RESIDENTIAL_PROXY_ENV_ENABLED = "A0_BROWSER_RESIDENTIAL_PROXY_ENABLED"
BASE_BROWSER_ARGS = [
    "--no-sandbox",
...
def _normalize_residential_proxy_server(value: Any) -> str:
    server = str(value or "").strip()
    return server or DEFAULT_RESIDENTIAL_PROXY_SERVER
...
def normalize_browser_config(settings: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        RESIDENTIAL_PROXY_ENABLED_KEY: _normalize_bool(
            raw.get(RESIDENTIAL_PROXY_ENABLED_KEY, DEFAULT_RESIDENTIAL_PROXY_ENABLED),
            default=DEFAULT_RESIDENTIAL_PROXY_ENABLED,
        ),
        RESIDENTIAL_PROXY_SERVER_KEY: _normalize_residential_proxy_server(
            raw.get(RESIDENTIAL_PROXY_SERVER_KEY, DEFAULT_RESIDENTIAL_PROXY_SERVER)
        ),
    }
...
def get_residential_proxy_config(
    ...
    """Resolve residential-proxy escalation config.

    Precedence: environment override > plugin settings > built-in default.
    ...
    enabled = bool(
        config.get(RESIDENTIAL_PROXY_ENABLED_KEY, DEFAULT_RESIDENTIAL_PROXY_ENABLED)
    )
    server = str(
        config.get(RESIDENTIAL_PROXY_SERVER_KEY, DEFAULT_RESIDENTIAL_PROXY_SERVER) or ""
    ).strip()

    env_enabled = os.environ.get(RESIDENTIAL_PROXY_ENV_ENABLED)
    if env_enabled is not None and env_enabled.strip() != "":
        enabled = _normalize_bool(env_enabled, default=enabled)
    env_server = os.environ.get(RESIDENTIAL_PROXY_ENV_SERVER)
    if env_server and env_server.strip():
        server = env_server.strip()

    if not server:
        server = DEFAULT_RESIDENTIAL_PROXY_SERVER
